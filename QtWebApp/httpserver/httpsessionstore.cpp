...
HttpSessionStore::HttpSessionStore(
	const HttpSessionStoreConfig &cfg, QObject *parent)
	: QObject(parent)
	, cfg(cfg)
{
...
QByteArray HttpSessionStore::getSessionId(
	HttpRequest &request, HttpResponse &response)
{
	...
	QByteArray sessionId =
		response.getCookies().value(cfg.cookieName).getValue();
	...
	if (!sessionId.isEmpty())
	{
		...
		{
			qDebug(
				"HttpSessionStore: received invalid session cookie with ID %s",
				sessionId.data());
			sessionId.clear();
		}
	}
...
HttpSession HttpSessionStore::getSession(
	HttpRequest &request, HttpResponse &response, bool allowCreate)
{
