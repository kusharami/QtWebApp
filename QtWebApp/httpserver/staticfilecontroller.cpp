...
StaticFileController::StaticFileController(
	const StaticFileControllerConfig &cfg, QObject *parent)
	: HttpRequestHandler(parent)
{
	...
	if (!cfg.fileName.isEmpty() &&
		!(docroot.startsWith(":/") || !docroot.startsWith("qrc://")))
	{
		// Convert relative path to absolute, based on the directory of the config file.
		if (QDir::isRelativePath(docroot))
		{
			QFileInfo configFile(cfg.fileName);
			docroot = QFileInfo(configFile.absolutePath(), docroot)
						  .absoluteFilePath();
		}
	}
#ifdef CMAKE_DEBUG
	qDebug("StaticFileController: docroot=%s, encoding=%s, maxAge=%i",
...
void StaticFileController::service(HttpRequest &request, HttpResponse &response)
{
	...
	if (entry && (cacheTimeout == 0 || entry->created > now - cacheTimeout))
	{
		QByteArray document =
			entry
				->document; // copy the cached document, because other threads may destroy the cached
		...
		response.setHeader(
			"Cache-Control", "max-age=" + QByteArray::number(maxAge / 1000));
	...
	} else
	{
		...
		if (path.contains("/.."))
		{
			qWarning("StaticFileController: detected forbidden characters in "
					 "path %s",
				path.data());
			...
			if (file.size() <= maxCachedFileSize)
			{
				...
				etag.insert(path,
					QCryptographicHash::hash(
						entry->document, QCryptographicHash::Md5)
						.toHex());
				response.setHeader("ETag", "\"" + etag.value(path) + "\"");
		...
		} else
		{
			if (file.exists())
			{
				qWarning("StaticFileController: Cannot open existing file %s "
						 "for reading",
...
void StaticFileController::setContentType(
	const QString &fileName, HttpResponse &response) const
{
	...
	{
		response.setHeader(
			"Content-Type", qPrintable("text/plain; charset=" + encoding));
	} else if (fileName.endsWith(".html") || fileName.endsWith(".htm"))
	{
		response.setHeader(
			"Content-Type", qPrintable("text/html; charset=" + encoding));
