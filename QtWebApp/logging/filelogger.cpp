...
void FileLogger::refreshSettings()
{
	...
	timestampFormat =
		settings->value("timestampFormat", "yyyy-MM-dd hh:mm:ss.zzz")
			.toString();
	...
	} else if (minLevelStr == "WARNING" || minLevelStr == "WARN" ||
		minLevelStr == "1")
	{
		minLevel = QtMsgType::QtWarningMsg;
	} else if (minLevelStr == "ERROR" || minLevelStr == "CRITICAL" ||
		minLevelStr == "2")
	{
...
FileLogger::FileLogger(
	QSettings *settings, const int refreshInterval, QObject *parent)
	: Logger(parent)
{
...
void FileLogger::write(const LogMessage *logMessage)
{
	...
	if (file)
	{
		// Write the message
		file->write(
			qPrintable(logMessage->toString(msgFormat, timestampFormat)));
